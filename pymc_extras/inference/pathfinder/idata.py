...
def multipathfinder_result_to_xarray(
    ...
    *,
    ...
    summary_ds = _build_summary_dataset(result, param_coords, LBFGSStatus, PathStatus)

    paths_ds = None
    if not result.all_paths_failed and result.samples is not None:
        paths_ds = _build_paths_dataset(result, param_coords, n_params)
...
def _build_paths_dataset(
    ...
    if n_params is not None and result.samples is not None and result.samples.ndim >= 3:
        ...
        data_vars["final_sample"] = xr.DataArray(
            final_samples, dims=["path", "param"], coords=coords
        )
...
def _build_diagnostics_dataset(
    result: MultiPathfinderResult, model: pm.Model | None = None
) -> xr.Dataset | None:
    ...
    if result.samples is not None and result.samples.ndim == 3 and param_coords is not None:
        ...
        data_vars["samples_full"] = xr.DataArray(
            result.samples, dims=["path", "draw_per_path", "param"], coords=coords
        )
    ...
    if is_multipath:
        ...
        if paths_ds is not None:
            if paths_group in idata.groups():
                warnings.warn(
                    f"Group '{paths_group}' already exists in InferenceData, it will be replaced."
                )
        ...
        if store_diagnostics:
            ...
            if diagnostics_ds is not None:
                if diagnostics_group in idata.groups():
                    warnings.warn(
                        f"Group '{diagnostics_group}' already exists in InferenceData, it will be replaced."
                    )
        ...
        if result.pathfinder_config is not None:
            ...
            if config_group in idata.groups():
                warnings.warn(
                    f"Group '{config_group}' already exists in InferenceData, it will be replaced."
                )
