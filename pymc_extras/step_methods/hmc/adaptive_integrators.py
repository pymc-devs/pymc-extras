...
def fixedLeapFrog(q, v, g, Ham0, h, xi, lpFun, delta, auxPar):
    ...
    g_clipped = np.clip(g, -1e3, 1e3)

    vh = xi * v + 0.5 * h * g_clipped
    qq = q + h * vh
    fnew, gnew = lpFun(qq)

    # Clip new gradients as well
    gnew_clipped = np.clip(gnew, -1e3, 1e3)
    vv = vh + 0.5 * h * gnew_clipped

    # Compute Hamiltonian (let natural overflow/underflow occur)
    H1 = -fnew + 0.5 * sum(vv * vv)

    # Compute igrConst with safeguards
    ham_diff = abs(Ham0 - H1) if np.isfinite(Ham0) and np.isfinite(H1) else 1.0
    igrConst = h * (max(1.0e-10, ham_diff) ** (-1.0 / 3.0))
