...
def stopCondition(qm, vm, qp, vp):
    """NUT stop condition with overflow protection."""
    tmp = qp - qm

    # Protect against overflow in dot products
    try:
        dot1 = sum(vp * tmp)
        dot2 = sum(vm * tmp)

        # Check if values are finite
        if not (np.isfinite(dot1) and np.isfinite(dot2)):
            return True  # Conservative: assume U-turn if overflow

        return dot1 < 0.0 or dot2 < 0.0
    except (OverflowError, FloatingPointError):
        return True  # Conservative: assume U-turn if any numerical error
...
class WALNUTS(BaseHMC):
    ...
    **kwargs
    ...
    def __init__(
        self,
        vars=None,
        H0=0.01,  # Much smaller default step size for stability
        stepSizeRandScale=0.1,  # Less randomization
        delta0=0.1,  # Larger tolerance to be more forgiving
        M=10,
        integrator=None,
        igrAux=None,
        adaptH=True,
    ...
    def _hamiltonian_step(self, start, p0, step_size):
        ...
        # Auto-reduce step size if we're getting too many rejections
        if hasattr(self, "_consecutive_rejects"):
            if self._consecutive_rejects > 5:
                H = H * 0.5  # Halve step size
                self.H = H  # Update stored value
                self._consecutive_rejects = 0
                if self.tune:
                    import warnings

                    warnings.warn(f"Reducing step size to {H} due to numerical issues")
        else:
            self._consecutive_rejects = 0
        ...
        if np.any(np.abs(grad0) > 1e6):
            if self.tune:  # Only warn during tuning
                import warnings

                warnings.warn(
                    f"Extreme gradients detected: max abs gradient = {np.max(np.abs(grad0)):.2e}"
                )

        # Clip extreme gradients to maintain numerical stability
        grad0 = np.clip(grad0, -1e3, 1e3)
        ...
        if not np.isfinite(f0) or f0 < -1e10:
            if self.tune:
                import warnings

                warnings.warn(f"Extreme initial log probability: {f0}")
            # Force rejection if initial state is extremely bad
            forcedReject = True
            stopCode = 997  # Bad initial state
        ...
        for i in range(self.M):
            ...
            if i == 0:  # Single first integration step required
                ...
                if xi == 1:  # Forward integration
                    ...
                    if warmup and self.adaptH:
                        # Protect against log(0) which produces -inf
                        if intOut.igrConst > 0 and np.isfinite(intOut.igrConst):
            ...
            # Check for numerical instability before next doubling
            if (
                not np.all(np.isfinite(vp))
                or not np.all(np.isfinite(vm))
                or not np.all(np.isfinite(qp))
                or not np.all(np.isfinite(qm))
            ):
                forcedReject = True
                stopCode = 998  # Numerical instability
                break
        ...
        # If forcedReject due to bad initial state, return early
        if forcedReject and stopCode == 997:
            ...
            stats = {
                # Minimal stats for rejected sample
                "depth": 0,
                "step_size": self.H,
                "tune": self.tune,
                "mean_tree_accept": 0.0,
                "step_size_bar": self.H,
                "tree_size": 0,
                "diverging": True,
                "energy_error": np.inf,
                "energy": final_state.energy,
                "max_energy_error": np.inf,
                ...
                "n_steps_total": 0,
                "avg_steps_per_proposal": 0.0,
                "largest_eigval": np.nan,
                "smallest_eigval": np.nan,
            }

            return HMCStepData(final_state, 0, divergence_info, stats)
        ...
        if warmup:
            ...
            if self.adaptDelta:
                # Protect against overflow and division by zero
                if self.delta > 0 and np.isfinite(orbitEnergyError) and np.isfinite(self.delta):
                    ratio = orbitEnergyError / self.delta
                    # Clamp the ratio to prevent extreme values
                    self.energyErrorInfFacs[self.iterN - 1] = np.clip(ratio, 0.0, 1e6)
                else:
                    self.energyErrorInfFacs[self.iterN - 1] = 1e6  # Large but finite value
        ...
        return HMCStepData(final_state, 1 if not forcedReject else 0, divergence_info, stats)
